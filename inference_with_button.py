#inference_with_button.py

# includes code generated by chatgpt to implement button functionality using threads
#     - haven't tested it yet; may or may not work
# pushbutton code is at the end of the file
...
####################################################################################################
############ code generated by chatgpt to implement button functionality using threads ############
####################################################################################################
BUTTON_GPIO_NUM = "60"
...
def watch_button():
    global inference_enabled, inference_thread

    with open(GPIO_PATH, "r") as gpio_fd:
        poller = select.poll()
        poller.register(gpio_fd, select.POLLPRI)
        gpio_fd.read()  # clear initial event

        print("Waiting for button press to toggle inference...")

        while True:
            events = poller.poll(100)  # 100 ms timeout
            ...
            if events:
                ...
                print(f"\n🔁 Inference toggled {'ON' if inference_enabled else 'OFF'}")

                if inference_enabled:
                    # Start inference loop
                    inference_thread = threading.Thread(target=run_main_loop)
                    inference_thread.start()
                else:
                    # Wait for current inference cycle to finish
...
if __name__ == "__main__":
    setup_gpio_button()
    watch_button()  # This is the loop that waits for button presses and toggles inference
